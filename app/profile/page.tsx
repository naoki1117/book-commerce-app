...
import { ItemType, PurchaseBook, User } from "../types/types";
import { getDetailBook } from "../lib/microcms/client";
import PurchasesDetailBook from "../commponents/PurchasesDetailBook";
...
export default async function ProfilePage() {
  ...
  if (user) {
    ...
    purchasesDetailBooks = await Promise.all(
      purchasesData.map(async (purchase: PurchaseBook) => {
      ...
      <div className="flex items-center gap-6">
        {purchasesDetailBooks.map((purchasesDetailBook: ItemType) => (
          <PurchasesDetailBook
            ...
            detailBook={purchasesDetailBook}
          />
        ))}
      </div>
    </div>
